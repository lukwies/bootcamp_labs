...
def cluster_song(df):
	...
	# Load scaler
	with open('../scalers/standard.pickle', 'rb') as file:
...
#		model = pickle.load(file)
	...
	with open('../models/kmeans_11.pickle', 'rb') as file:
		model = pickle.load(file)
...
def recommend_song(song, cluster, n_recommend=1):
	...
	print("\nRecommended:")
	if is_hot:
		reco = df[(df['hot'] == 1) & (df['cluster_kmeans_11'] == cluster)].sample(n_recommend)
		reco.apply(show_recommendation, axis=1)
	else:
		reco = df[(df['hot'] == 0) & (df['cluster_kmeans_11'] == cluster)].sample(n_recommend)
		reco.apply(show_recommendation, axis=1)
